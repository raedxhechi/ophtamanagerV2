...
export function DatePicker({ date, setDate, onlyFutureDates, label }: DatePickerProps) {
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant={'outline'}
          className={cn(
            'w-full justify-between text-left font-normal bg-white text-gray-600',
            !date && 'text-muted-foreground'
          )}
        >
          {date ? format(date, 'PPP') : <span>{!!label ? label : 'Wähle ein Datum'}</span>}
      ...
      <PopoverContent className='w-auto p-0' align='start'>
        ...
        <Calendar
          ...
          disabled={(date) =>
            onlyFutureDates ? date < new Date() : date > new Date() || date < new Date(1900, 0, 1)
          }
        />
      </PopoverContent>
    </Popover>
